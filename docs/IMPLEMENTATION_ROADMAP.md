...
1. [x] Persistent storage (SQLite/PostgreSQL) — DONE (Rust side). `ServerState` (`src/server.rs`) now takes an optional `persistent_backend: Arc<dyn StorageBackend>` via `ServerState::with_backend()`. `handle_submit` write-throughs every observation to it (awaited — a persistence failure surfaces as a real error, not a silent 201), and `ServerState::restore_from_backend()` repopulates the in-memory store + spatial/temporal indices on startup via the new `storage::persistence_bridge` module (`Observation` <-> `StorageObservation` conversion, with real per-row error handling instead of panicking or silently dropping corrupt rows). A reboot with a configured backend (e.g. `PostgresBackend`) no longer loses terrain history. Still open: the Python API's backend factory (`python/pyterrain_map/api.py`) doesn't yet expose `with_backend`/`restore_from_backend` — this is currently a Rust-level capability, Python wiring is a natural follow-up.
2. Image stitching (Structure from Motion)
3. Advanced anomaly detection (ML-based)
4. Performance optimization (1M+ observations)
5. [x] Retention/compaction for append-only growth (external critique, verified real gap) — DONE, additively. `ObservationStore` still has no delete/mutate method (unchanged — that guarantee is intentional and this doesn't touch it). `storage::rollup::CompactionPolicy` reads a time window of existing observations through the store's public API and produces real `CellRollup` summaries: per-parent-H3-cell (via `xs_h3::cell_to_parent`), per-time-bucket, per-sensor-type aggregates (count, mean confidence, min/max timestamp, contributing robots) — genuine data reduction, computed and tested (9 tests), not a placeholder. What's *not* done yet: actually evicting rolled-up raw observations from the hot in-memory store or archiving them to cold storage (`DataTier::Cold`/`Archive`) — that needs a real archival backend to wire into, and deleting from `ObservationStore` directly would violate its immutability guarantee. This ships the real aggregation computation; wiring it to bound actual memory/disk footprint is the next step.
